...
class FinalLoad(object):
    ...
    def _fetch_batch_data(self, flag='train'):
        t1 = time.time()
        if flag == 'train':
        ...
        comp_list, y_list = self._concate_map_id(data_id, existing_id, self.user_map, self.item_map, self.field_num,
        ...
        # csc_comp_list = self._convert_onehot_csc_pop(comp_list, pop_list, self.mask_field_id)
        # print('%s dataset fetch batch data@%.4f' % (flag, time.time() - t1))

        return comp_list, y_list
    ...
    def _concate_map_id(self, data_id, existing_id, user_map, item_map, field_num, user_num, mask_field_id):
        ...
        data_id = self._padding_id(data_id, existing_id, field_num, user_num, mask_field_id)
...
if __name__ == '__main__':
    ...
    train_comp_list, train_y_list = my_load._fetch_batch_data(flag='train')

    test_comp_list, test_y_list = my_load._fetch_batch_data(flag='test')